...
frappe.ui.form.on("Policy Register Staging", {
	refresh(frm) {
		...
		add_policy_register_staging_actions(frm);
		register_policy_register_staging_form_realtime();
	},
...
const POSTABLE_POLICY_REGISTER_VALIDATION_STATUSES = ["Valid"];

// This adds actions that queue validation or posting for only the open staging record.
function add_policy_register_staging_actions(frm) {
	if (frm.is_new() || !is_policy_register_staging_actionable(frm.doc)) {
		return;
	}

	frm.add_custom_button(__("Validate This Record"), () => {
		queue_policy_register_staging_action({
			frm,
			method: POLICY_REGISTER_STAGING_METHODS.validate,
			freeze_message: __("Starting validation for this record..."),
			title: __("Validation Started"),
		});
	});

	if (!is_policy_register_staging_postable(frm.doc)) {
		return;
	}
	...
	frm.add_custom_button(__("Post This Record"), () => {
		frappe.confirm(
			__(
				"This will create and submit the final Policy Register for this staging record. Continue?"
			),
			() => {
				queue_policy_register_staging_action({
					frm,
					method: POLICY_REGISTER_STAGING_METHODS.post,
...
function is_policy_register_staging_actionable(doc) {
	return (
		!cint(doc.ignore_record) &&
		!doc.posted_policy_register &&
...
function is_policy_register_staging_postable(doc) {
	return (
		is_policy_register_staging_actionable(doc) &&
		POSTABLE_POLICY_REGISTER_VALIDATION_STATUSES.includes(doc.validation_status) &&
		doc.processing_status === "Ready"
	);
}

// This queues one staging record through the existing background processing methods.
function queue_policy_register_staging_action({ frm, method, freeze_message, title }) {
	frappe.call({
		method,
		args: {
...
function register_policy_register_staging_form_realtime() {
	if (window.policy_register_staging_form_listener_registered) {
		return;
	}

	window.policy_register_staging_form_listener_registered = true;

	frappe.realtime.on("policy_register_staging_job_complete", (data) => {
		const frm = cur_frm;

		if (
			!frm ||
			frm.doctype !== "Policy Register Staging" ||
